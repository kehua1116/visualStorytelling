# START: COPIED FROM <https://github.com/lichengunc/vist_api>
...
# 		img_content = resize(img_content, (500, 500))
		return img_content


# END: COPIED FROM <https://github.com/lichengunc/vist_api>

	def show_story(self, album_id, story_id, show_image=True, show_sents=True):
		cnn_bart_path = "../res/baseline_result_epoch10_nocat.txt"
		cnn_bart_concept_path = "../res/baseline_result_concept.txt"
		
		cnn_bart_prediction = json.load(open(cnn_bart_path))
		cnn_bart_concept_prediction = json.load(open(cnn_bart_concept_path))

		story = self.Stories[story_id]
		sent_ids = story['sent_ids']
		cnn_bart_predict_sents = cnn_bart_prediction[album_id][0].split(".")
		cnn_bart_concept_predict_sents = cnn_bart_concept_prediction[album_id][0].split(".")
		...
		if show_sents:
			...
			for sent_id in sent_ids:
				...
				print('%s: %s' % (sent['order'], sent['text']))
			print("\n")
			print("CNN + BART Predicted Sentences:")
			for i in range(len(cnn_bart_predict_sents)):
				if len(cnn_bart_predict_sents[i]) > 0:
					print('%s: %s' % (i, cnn_bart_predict_sents[i].strip()))
			print("\n")
			print("CNN + Concept + BART Predicted Sentences:")
			for i in range(len(cnn_bart_concept_predict_sents)):
				if len(cnn_bart_concept_predict_sents[i]) > 0:
					print('%s: %s' % (i, cnn_bart_concept_predict_sents[i].strip()))
	

	def show_test_story(self, album_id, story_id, evaluation_scripts, show_image=True, show_sents=True):
		all_preds = []
		for cnn_bart_path in evaluation_scripts:	
			cnn_bart_prediction = json.load(open(cnn_bart_path))

			story = self.Stories[story_id]
			sent_ids = story['sent_ids']
			if len(cnn_bart_prediction[album_id]) > 1: # #contain emotion words
				cnn_bart_predict_sents = cnn_bart_prediction[album_id][1].split(".")
			else:
				cnn_bart_predict_sents = cnn_bart_prediction[album_id][0].split(".")
			all_preds.append(cnn_bart_predict_sents)
		...
		if show_image:
			...
			plt.show()
		if show_sents:
			print(f"Album:{album_id}, Story:{story_id}")
			for i, model_pred in enumerate(all_preds):
				print(f"Model {i}")
				for j in range(len(model_pred)):
					if len(model_pred[j]) > 0:
						print('%s: %s' % (j, model_pred[j].strip()))
				print("\n")
...
class Description_in_Isolation:
	...
	def read_img(self, img_file):
		...
		if len(img_content.shape) == 2:
			img_content = np.tile(img_content[:,:,np.newaxis], (1,1,3))
		img_content = resize(img_content, (224, 224))
		return img_content
